...
2. Fetch the source tree as a tarball via the github MCP `get_repo_archive`
   (the token stays host-side; works for private repos). To keep large blobs
   out of the model context, fetch straight to a file with the sandbox
   `mcp-client` CLI and extract (replace `<branch>` with the default branch):
