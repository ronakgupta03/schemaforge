import { parseVerify, migrationLabel } from "../verify";
...
function Row({
  ...
  const color =
    ok === true ? "var(--sf-ok)" : ok === false ? "var(--sf-fail)" : "var(--sf-muted)";
  return (
    <div className="border rounded p-2" style={{ borderColor: "var(--sf-border)" }}>
      ...
      {output && (
        <details className="text-xs mt-1" style={{ color: "var(--sf-muted)" }}>
  ...
  return (
    <div className="space-y-2 p-4">
      ...
      <div className="text-sm font-semibold pt-2">
        Query performance (EXPLAIN ANALYZE, ms)
      </div>
      <table className="w-full text-xs" style={{ color: "var(--sf-text)" }}>
        <thead>
          <tr className="text-left" style={{ color: "var(--sf-muted)" }}>
            <th>query</th>
            <th>before</th>
            <th>after</th>
          </tr>
      ...
      <div className="text-xs" style={{ color: "var(--sf-muted)" }}>
        Schema diff: {JSON.stringify(v.diff)}
      </div>
    </div>
  );
}
